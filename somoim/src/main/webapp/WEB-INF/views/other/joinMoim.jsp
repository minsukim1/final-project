...
#other-join-moim .home-body {
	font-size: 18px;
	margin-left: 3px;
	padding: 35px;
}
#other-join-moim .home-card {
 	width: 345px;
 	height: 330px;
 	margin: 6px;
}
	...
	<div class="col-12">
		<nav class="navbar">
			<ul class="nav nav-tabs">
				...
				<li class="nav-item"><a class="nav-link active" href="/other/usermoim.do">가입모임</a></li>
...
<div class='clearfix' id="other-join-moim">
	<c:choose>
		<c:when test="${followerYN eq 0 }">
			<div><p>팔로워 하지 않으면 볼 수 없습니다.</p></div>
		</c:when>
		<c:when test="${empty joinmoim }">
			<div><p>내가 가입한 모임이 존재하지않습니다.</p></div>
		</c:when>
		<c:otherwise>
			<c:forEach items="${joinmoim }" var="moim">
